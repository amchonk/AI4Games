...
class AIState(Enum):
    HUNT = 1 # Added hunt state for AI player
    COLLECT = 2 # Added collect state for AI player
    DODGE = 3 # Added dodge state for AI player
    IDLE = 4  # Added idle state for AI player
...
class AIPlayer(Player):
    def __init__(self):
        super().__init__()
        self.state = AIState.COLLECT  # Start with collecting as the default state
        self.target = None

    def update(self):
        super().update()  # Continue with the basic player mechanics

        if self.detect_danger(): 
            self.state = AIState.DODGE # If we see a bolt, dodge it
        elif self.see_fruit(): # If we see fruit, collect it
            self.state = AIState.COLLECT # New check for fruit directly in front
        elif self.see_fruit_below():  # New check for fruit directly below
            self.drop_to_fruit() # Drop to fruit if it's below
        else:
            self.state = AIState.HUNT # If we don't see fruit or danger, hunt the robot

        if self.state == AIState.HUNT:  # If we're hunting, we need to choose a target
            self.hunt()
        elif self.state == AIState.COLLECT: # If we're collecting, we need to move towards the fruit
            self.collect_fruit()
        elif self.state == AIState.DODGE: # If we're dodging, we need to move away from the bolt
            self.dodge()

    def see_fruit(self):
        for fruit in game.fruits: 
            if abs(fruit.x - self.x) < 200 and abs(fruit.y - self.y) < 100: # Check for fruit within a certain range
                return True
        return False

    def see_fruit_below(self):
        for fruit in game.fruits: # Check for fruit directly below
            if fruit.x == self.x and fruit.y > self.y: 
                if not any(block(self.x, self.y + GRID_BLOCK_SIZE * i) for i in range(1, (fruit.y - self.y) // GRID_BLOCK_SIZE)): # Check for blocks between player and fruit
                    return True # If there are no blocks, return True
        return False # If no fruit is found, return False

    def drop_to_fruit(self):
        self.vel_y = GravityActor.MAX_FALL_SPEED  # Set falling speed to max to drop quickly

    def collect_fruit(self):
        if game.fruits:
            nearest_fruit = min(game.fruits, key=lambda f: abs(f.x - self.x) + abs(f.y - self.y)) # Find nearest fruit
            self.move_toward(nearest_fruit.x, nearest_fruit.y) # Move towards nearest fruit
            if abs(nearest_fruit.y - self.y) > 50: # If the fruit is more than 50 pixels away vertically, jump
                self.jump_or_drop(nearest_fruit) # Jump or drop to fruit
        else:
            self.idle_or_patrol()

    def jump_or_drop(self, fruit): # Jump or drop to fruit depending on its position
        if fruit.y < self.y: # If fruit is above player, jump
            self.jump() 
        elif fruit.y > self.y: # If fruit is below player, drop
            self.drop_to_fruit()

    def idle_or_patrol(self):
        # Basic patrolling logic: move back and forth between two points
        if not hasattr(self, 'patrol_bounds'):
            # Set patrol bounds relative to the current position if not set
            self.patrol_bounds = (max(70, self.x - 100), min(730, self.x + 100))

        if self.direction_x == 0:
            self.direction_x = 1  # Default start moving right if not moving

        # Move in the current direction
        self.move(self.direction_x, 0, 4)

        # Check if we've reached the patrol bounds to reverse direction
        if self.x <= self.patrol_bounds[0] or self.x >= self.patrol_bounds[1]:
            self.direction_x *= -1  # Reverse direction


    def detect_danger(self): # Check for bolts in the game
        for bolt in game.bolts:
            if bolt.collidepoint((self.x, self.y)): # If bolt is close to player
                return True
        return False

    def hunt(self): # Hunt the robot
        if not self.target or not self.target.alive: # If there's no target or the target is dead
            self.target = self.choose_target() # Choose a new target
        if self.target: # If there is a target
            self.move_toward(self.target.x, self.target.y) # Move towards the target
            if abs(self.target.y - self.y) > 50: # If the target is more than 50 pixels away vertically, jump
                self.jump() 
            self.shoot() # Shoot at the target

    def dodge(self): # Dodge the bolt
        if game.bolts:
            threat = min(game.bolts, key=lambda b: abs(b.x - self.x)) # Find the nearest bolt
            self.move_away(threat.x, threat.y) # Move away from the bolt
            if abs(threat.y - self.y) > 50: # If the bolt is more than 50 pixels away vertically, jump
                self.jump() 

    def move_toward(self, x, y): # Move towards a point
        self.direction_x = 1 if x > self.x else -1 # Set direction based on x position
        self.move(self.direction_x, 0, 4) # Move in the direction

    def move_away(self, x, y): # Move away from a point
        self.direction_x = -1 if x > self.x else 1 # Set direction based on x position
        self.move(self.direction_x, 0, 4) # Move in the direction

    def choose_target(self):
        return min(game.enemies, key=lambda e: abs(e.x - self.x) + abs(e.y - self.y), default=None) # Find the nearest enemy

    def shoot(self):
        if self.fire_timer <= 0:
            self.fire_timer = 20
            game.orbs.append(Orb((self.x + self.direction_x * 38, self.y - 35), self.direction_x)) # Create an orb

    def jump(self):
        if self.vel_y == 0 and self.landed:
            self.vel_y = -16  # Velocity for jumping
            self.landed = False 
            game.play_sound("jump")
...
class Game:
    def __init__(self, player=None):
        if player is None:
            self.player = AIPlayer()  # Instantiate AI player by default
        else:
            self.player = player
        self.level_colour = -1
        self.level = -1
